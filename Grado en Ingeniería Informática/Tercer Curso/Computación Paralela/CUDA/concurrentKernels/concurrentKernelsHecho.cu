...
__global__ void clock_block(clock_t *d_t, clock_t clock_count, int idx)
{
    ...
    d_t[idx] = clock_offset;  // ahora cada kernel escribe en su propio índice
}
...
int main(int argc, char **argv)
{
    ...
    // COMPLETAR...
    cudaMallocHost((void **)&h_data, nbytes);
    ...
    cudaStream_t *streams = (cudaStream_t *)malloc(nstreams * sizeof(cudaStream_t)); // COMPLETAR...

    // create streams
    for (int i = 0; i < nstreams; i++)
    {
        cudaStreamCreate(&streams[i]); // COMPLETAR...
    }
    ...
    for (int i = 0; i < nkernels; ++i)
    {
        clock_block<<<1, 1, 0, streams[i]>>>(&d_data[i], time_clocks,i); // COMPLETAR...
    ...
    // COMPLETAR...
    cudaMemcpyAsync(h_data, d_data, nbytes, cudaMemcpyDeviceToHost, streams[nstreams - 1]);
    ...
    bool bTestResult = (elapsed_time < nkernels * kernel_time * 0.5f);
    ...
    for (int i = 0; i < nkernels; i++)
    {
        cudaEventDestroy(kernelEvent[i]); // COMPLETAR...
    }

    // destroy streams
    for (int i = 0; i < nstreams; i++)
    {
        cudaStreamDestroy(streams[i]); // COMPLETAR...
    }
