...
int main(int argc, char **argv)
{
	...
	*reduce_h = 0.0f;
  cudaMemset(clocks_d, 0, 2 * nblocks * sizeof(clock_t));
  cudaMemcpy(reduce_d, reduce_h, sizeof(float), cudaMemcpyHostToDevice);
    ...
    size_t sharedMemSize = bsx * sizeof(float); // cantidad de memoria compartida por bloque
    time_and_reduce<<< grid, block, sharedMemSize >>>(vector_d, reduce_d, clocks_d, n);  // COMPLETAR...
	...
	printf("Resultado GPU: %.5f, Esperado: %.5f\n", reduce_h[0], acum);
