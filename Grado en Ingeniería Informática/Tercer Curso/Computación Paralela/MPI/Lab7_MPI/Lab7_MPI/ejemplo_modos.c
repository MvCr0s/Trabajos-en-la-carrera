...
int main(int argc, char *argv[]) {
	...
	if ( data == NULL || data2 == NULL ) {
		fprintf(stderr, "Rank %d, Error allocating memory for %s elements\n", rank, size );
		MPI_Abort( MPI_COMM_WORLD, -1 );
	}
	...
	int rank_from = (rank - jump + nprocs) % nprocs;
	...
	MPI_Send(data, size, MPI_DOUBLE, rank_to, tag, MPI_COMM_WORLD);

	// Receive in data2 array from the "rank_from" process
	MPI_Recv(data2, size, MPI_DOUBLE, rank_from, tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
