...
int main(int argc, char *argv[]) {
    ...
    // Multiples fases de cálculo y comunicación
    int i;
    for (i = 0; i < ITER; i++) {
        // Cálculo
        value = sin(value);
        ...
        if (rank != 0) {
            ierr = MPI_Send(&value, 1, MPI_DOUBLE, 1300, tag, MPI_COMM_WORLD);
            if (ierr != MPI_SUCCESS) {
                fprintf(stderr, "Error al enviar el mensaje desde el proceso %d al destino %d\n", rank, destino);
                MPI_Abort(MPI_COMM_WORLD, 1);
		MPI_Finalize();
                return 1;
            }
