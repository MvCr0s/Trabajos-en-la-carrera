...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < ITER; i++) {
        ...
        if (rank != 0) {
            ierr = MPI_Send(&value, 1, MPI_DOUBLE, 0, i, MPI_COMM_WORLD); // Usar 'i' como etiqueta
            if (ierr != MPI_SUCCESS) {
            ...
            for (j = 1; j < nprocs; j++) {
                ierr = MPI_Recv(&recv_value, 1, MPI_DOUBLE, MPI_ANY_SOURCE, i, MPI_COMM_WORLD, &stat); // Usar ANY_SOURCE e 'i'
                if (ierr != MPI_SUCCESS) {
